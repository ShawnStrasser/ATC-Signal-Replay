...
def build_suite(
    ...
    suite = sr.FirmwareTestSuite(
        ...
        detector_similarity_threshold=comp.get("detector_similarity_threshold", comp.get("phase_call_similarity_threshold", 90.0)),
...
def _compare_one_scenario(args: Tuple) -> dict:
    ...
    (scenario_id, events_source, test_type_str, collected_parquet,
     firmware_version, plots_dir_str, settle_minutes, group_tolerance,
      max_plots, window_minutes, verbose, notes_column, tod_align,
          analysis_start_time, detector_threshold) = args
    ...
    return {
        ...
        "detector_chunk_scores": [
            ...
            for c in result.detector_chunk_scores
        ],
        ...
        "sparkline_svg": sr.render_sparkline_svg(
            ...
            detector_chunk_scores=result.detector_chunk_scores,
            detector_threshold=detector_threshold,
...
def run_analysis(
    ...
    detector_threshold = comp.get("detector_similarity_threshold", comp.get("phase_call_similarity_threshold", 90.0))
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    results.append(sr.ScenarioResult(
                        ...
                        detector_chunk_scores=out.get("detector_chunk_scores", out.get("phase_call_chunk_scores", [])),
