...
|---|---|---|
| Twenty-five approximately 23-hour field traces were used | `firmware_validation/logs/*.parquet`; `paper/generated/results/run_metadata.json`; `dataset_summary.csv` | 7,296,218 raw records; approximately 23 hours per controller, or 575 hours summed across all 25 controllers. Only detector, pedestrian, and preempt events become replay calls. |
...
| Replay timing preserves field timing and compensates latency | `src/signal_replay/replay.py`; `src/signal_replay/latency.py`; stored latency tables | MAXTIME 2.18.1 contains 707,458 latency samples and 6,577 applied updates. Isolated scheduled detector-on inputs are matched with controller-recorded events to adapt the per-controller offset as workstation and controller clocks drift. |
| DTW aligns discrete event traces | `src/signal_replay/comparison.py:660-741`, `1416-1490`, `2152-2520` | Timestamps define simultaneous event groups and initial alignment but do not enter the sequence cost. Local cost is Jaccard distance between ordered event-group contents; timing is scored separately. Rolling windows are 45 minutes advanced every 40 minutes with 60-second edge clips. |
...
| Known overlap clearance-setting changes were detected | `parameter_detection_summary.csv`; archived `2.18.1_trailing/report.html` | 11/14 changed flagged and 0/11 unchanged flagged. Three changed passes may lack exposure and are not automatically algorithmic false negatives. |
...
| Detailed release findings included the 13008 rail-preemption exit fix, a non-operational 2B049 transition-algorithm difference, a possible 2B049 transition-state issue, and expected 13008 dynamic red-clear extension | Project presentation, extracted figures, and author-provided case classifications | Both 13008 and 2B049 passed aggregate thresholds; detailed diagnostics therefore add information beyond pass/fail. |
