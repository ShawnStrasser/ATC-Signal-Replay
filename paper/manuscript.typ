...
Li et al. and the Idaho Transportation Department developed automated controller testing that used Extensible Markup Language (XML) scripts to activate inputs and verify predefined responses @li2008 @ahmed2010. Tung extended NTCIP-based automated testing across controller devices @tung2012 @tung2015. These systems established the value of repeatable automated tests. The present method differs by replaying real events recorded as they occurred at live field controllers and using a prior complete output trace as the expected response rather than requiring staff to author every input and assertion.
...
#figure(
  image("figures/alignment-options.pdf", width: 100%),
  caption: [Three candidate explanations of event-sequence alignment for author selection: (a) ordered event groups, (b) the same alignment with timestamps restored, and (c) an explicit alignment table. Sequence comparison uses event-group content; timing is evaluated separately after alignment.],
) <fig-alignment>
...
The package is open source and separates NTCIP inputs from product-specific outputs. The replay-input method is not tied to a controller brand: it can be applied to controllers that implement the required NTCIP objects. The current event-log collector reads MAXTIME logs; supporting another controller family requires adding its log interface, event-code mapping, and timestamp validation. MAXTIME is the implementation evaluated here, not a limitation of the underlying replay method.
= Limitations
