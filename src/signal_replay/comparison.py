...
@dataclass
class DetectorChunkScore:
    """Reliability score for vehicle detector input in a rolling chunk."""
...
class ComparisonResult:
    ...
    # Reliability of the observed vehicle detector inputs by chunk
    detector_chunk_scores: List[DetectorChunkScore] = field(default_factory=list)
    ...
    @property
    def phase_call_chunk_scores(self) -> List[DetectorChunkScore]:
        return self.detector_chunk_scores

    @phase_call_chunk_scores.setter
    def phase_call_chunk_scores(self, value: List[DetectorChunkScore]) -> None:
...
def _score_detector_window(
    ...
    """Return an input-similarity score for vehicle detectors in one chunk."""
...
def _compute_detector_chunk_scores(
    ...
    threshold: float,
    clip_sec: float = 60.0,
) -> List[DetectorChunkScore]:
    """Score vehicle detector input similarity on the same rolling chunk grid."""
    if not chunk_scores:
        return []

    detector_df_a = prepare_events_for_comparison(events_a, start_time_a, event_ids=DETECTOR_EVENT_IDS)
    detector_df_b = prepare_events_for_comparison(events_b, start_time_b, event_ids=DETECTOR_EVENT_IDS)

    if detector_df_a.empty and detector_df_b.empty:
        return [
            DetectorChunkScore(
    ...
    if abs(temporal_shift_seconds) > 0.05 and not detector_df_b.empty:
        detector_df_b = detector_df_b.copy()
        detector_df_b['time_delta'] = detector_df_b['time_delta'] + temporal_shift_seconds
        detector_df_b = detector_df_b[detector_df_b['time_delta'] >= 0].reset_index(drop=True)

    if trim_seconds_a > 0 and not detector_df_a.empty:
        detector_df_a = detector_df_a[detector_df_a['time_delta'] >= trim_seconds_a].copy()
        detector_df_a['time_delta'] = detector_df_a['time_delta'] - trim_seconds_a

    if trim_seconds_b > 0 and not detector_df_b.empty:
        detector_df_b = detector_df_b[detector_df_b['time_delta'] >= trim_seconds_b].copy()
        detector_df_b['time_delta'] = detector_df_b['time_delta'] - trim_seconds_b
    ...
    for chunk in chunk_scores:
        ...
        similarity_percentage, has_activity = _score_detector_window(
            detector_df_a,
            detector_df_b,
...
def _apply_detector_chunk_filter(
    chunk_scores: List[ChunkScore],
    detector_chunk_scores: List[DetectorChunkScore],
) -> Tuple[float, int, int]:
    """Average only chunks that are not flagged by the input-similarity sanity check."""
    if not chunk_scores:
        return 0.0, 0, 0

    if not detector_chunk_scores or len(detector_chunk_scores) != len(chunk_scores):
        return (
    ...
    excluded_count = 0
    for chunk, detector_chunk in zip(chunk_scores, detector_chunk_scores):
        if detector_chunk.excluded_from_match:
            excluded_count += 1
            continue
...
def render_sparkline_svg(
    ...
    detector_chunk_scores: Optional[List[DetectorChunkScore]] = None,
    detector_threshold: float = 90.0,
    ...
    """
    if not chunk_scores and not detector_chunk_scores:
        return ""
    ...
    domain_candidates = list(chunk_scores) + list(detector_chunk_scores or [])
    ...
    y_values.extend(
        ...
        for score in (detector_chunk_scores or [])
    ...
    detector_lookup = {
        ...
        for cs in (detector_chunk_scores or [])
    }
    ...
    for i, cs in enumerate(chunk_scores):
        ...
        detector_chunk = detector_lookup.get((round(cs.center_seconds, 6), round(cs.window_seconds, 6)))
        detector_text = "Detector similarity: n/a"
        opacity = "0.85"
        if detector_chunk is not None:
            if detector_chunk.similarity_percentage is not None:
                detector_text = f'Detector similarity: {detector_chunk.similarity_percentage:.1f}%'
            elif detector_chunk.has_activity:
                detector_text = 'Detector similarity: insufficient activity'
            if detector_chunk.excluded_from_match:
                opacity = "0.42"
        ...
        bars.append(
            ...
            f'<title>{tooltip_label}: Match {pct:.1f}% | {detector_text}</title></rect>'
        )

    detector_segments = []
    detector_points = []
    current_segment = []
    for detector_chunk in detector_chunk_scores or []:
        similarity = detector_chunk.similarity_percentage
        if similarity is None:
            if current_segment:
                detector_segments.append(current_segment)
                current_segment = []
            continue
        frac = max(0.0, min(1.0, (detector_chunk.center_seconds - domain_start) / domain_span))
        x = margin_left + frac * plot_w
        y = _y(similarity)
        current_segment.append((x, y, detector_chunk))

    if current_segment:
        detector_segments.append(current_segment)

    detector_lines_svg = []
    for segment in detector_segments:
        if len(segment) >= 2:
            points_attr = " ".join(f"{x:.1f},{y:.1f}" for x, y, _ in segment)
            detector_lines_svg.append(
                f'<polyline points="{points_attr}" fill="none" stroke="#000" stroke-width="1.2" />'
            )
        for x, y, detector_chunk in segment:
            window_start_seconds = max(domain_start, detector_chunk.center_seconds - (detector_chunk.window_seconds / 2.0))
            window_end_seconds = min(domain_end, detector_chunk.center_seconds + (detector_chunk.window_seconds / 2.0))
            if base_timestamp is not None:
                tooltip_start = base_timestamp + timedelta(seconds=window_start_seconds)
                tooltip_end = base_timestamp + timedelta(seconds=window_end_seconds)
                tooltip_label = (
            ...
            detector_points.append(
                ...
                f'<title>{tooltip_label}: Detector similarity {detector_chunk.similarity_percentage:.1f}%</title></circle>'
            )
    ...
    if detector_chunk_scores:
        legend_items.append(
            '<line x1="0" y1="6" x2="12" y2="6" stroke="#000" stroke-width="1.2" />'
            '<circle cx="6" cy="6" r="2.6" fill="#000" />'
            '<text x="18" y="10" font-size="12" fill="#1f1f1f" font-family="sans-serif">Detector similarity</text>'
        )
        if show_exclusion_legend:
            legend_items.append(
                f'<rect x="0" y="0" width="12" height="12" fill="#80868b" opacity="0.42" />'
                f'<text x="18" y="10" font-size="12" fill="#1f1f1f" font-family="sans-serif">Excluded from match average (&lt; {detector_threshold:.0f}%)</text>'
            )
    ...
    svg = (
        ...
        f'{"".join(detector_lines_svg)}{"".join(detector_points)}'
...
def compare_runs(
    ...
    detector_chunk_scores = _compute_detector_chunk_scores(
        chunk_scores,
        events_a=events_a,
        events_b=events_b,
    ...
    settled_detector_chunk_scores = _filter_chunk_scores_after_settle(detector_chunk_scores, settle_seconds)
    if settle_seconds > 0 and chunk_scores:
        ...
        if settled_chunks:
            match_percentage, included_chunk_count, excluded_chunk_count = _apply_detector_chunk_filter(
                settled_chunks,
                settled_detector_chunk_scores,
            )
        else:
            included_chunk_count = 0
            excluded_chunk_count = 0
    ...
    return ComparisonResult(
        ...
        detector_chunk_scores=settled_detector_chunk_scores,
