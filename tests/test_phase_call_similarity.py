...
def _make_events(main_offsets, detector_offsets=None):
    ...
    for event_id, offset, parameter in detector_offsets or []:
        rows.append(
            {
                "timestamp": base_time + timedelta(seconds=offset),
                "event_id": event_id,
...
def test_compare_runs_excludes_chunk_when_detector_similarity_is_below_threshold():
    ...
    events_a = _make_events(main_offsets, detector_offsets=detector_offsets_a)
    events_b = _make_events(main_offsets, detector_offsets=[])
    ...
    result = compare_runs(
        ...
        device_id="03013",
        auto_align=False,
        detector_threshold=90.0,
    )

    assert len(result.chunk_scores) == 1
    assert result.chunk_scores[0].match_percentage == 100.0
    assert len(result.detector_chunk_scores) == 1
    assert result.detector_chunk_scores[0].similarity_percentage == 0.0
    assert result.detector_chunk_scores[0].excluded_from_match is True
    assert result.included_chunk_count == 0
    assert result.excluded_chunk_count == 1
    assert result.match_percentage == 0.0


def test_compare_runs_does_not_exclude_low_activity_detector_chunk():
    main_offsets = [0, 600, 1200, 1800, 2400, 2800]
    detector_offsets_a = [(82, 900, 2)]
    ...
    assert len(result.detector_chunk_scores) == 1
    assert result.detector_chunk_scores[0].similarity_percentage is None
    assert result.detector_chunk_scores[0].has_activity is True
    assert result.detector_chunk_scores[0].excluded_from_match is False
    assert result.included_chunk_count == 1
    assert result.excluded_chunk_count == 0
    assert result.match_percentage == 100.0


def test_render_sparkline_draws_detector_overlay_and_exclusion_legend():
    svg = render_sparkline_svg(
        ...
        detector_chunk_scores=[
            DetectorChunkScore(center_seconds=1350.0, window_seconds=2700.0, similarity_percentage=92.0),
            DetectorChunkScore(center_seconds=3750.0, window_seconds=2700.0, similarity_percentage=84.0, excluded_from_match=True),
        ],
        detector_threshold=90.0,
    ...
    assert 'Detector similarity' in svg
...
def test_combined_timeline_chart_uses_dynamic_y_axis_without_exclusion_legend():
    svg = render_sparkline_svg(
        [],
        detector_chunk_scores=[
            DetectorChunkScore(center_seconds=1350.0, window_seconds=2700.0, similarity_percentage=91.8),
            DetectorChunkScore(center_seconds=3750.0, window_seconds=2700.0, similarity_percentage=93.1),
        ],
        detector_threshold=90.0,
        auto_scale_y=True,
...
def test_generate_report_includes_combined_timeline_and_threshold(tmp_path):
    suite = FirmwareTestSuite(
        ...
        detector_similarity_threshold=90.0,
    )
    ...
    assert "Vehicle detector similarity threshold" in html
    assert "90.0%" in html
